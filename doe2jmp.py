...
def makeDoeScript(fact, outfile, size, seed=None, starts=1040, executable=False, makeTable=False, makeFullFactorial=False, onlyCategorical=True):
    ...
    if executable:
        doe.append( '//!' )
    doe.append('// Script for DoE-based rapid prototyping of producer strains. Pablo Carbonell, SYNBIOCHEM, 2020.')
    doe.append('// Steps:')
    ...
    doe.append('// Note: A D-optimal design experiment (Custom Design) assuming categorical factors is generated by default. Other experiments can be selected in JMP, for instance:')
    doe.append('//  A. Ordered factors, i.e. weak vs strong promoters: replace the categorical factor by a discrete numeric factor.')
    doe.append('//  B. Another DoE method, i.e., factorial, definitive screening, response surface, mixture, etc: save the factors to a JMP table and load them into the new experiment.')
    doe.append( 'DOE(' )
