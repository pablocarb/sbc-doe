...
def arguments():
    ...
    parser.add_argument('outcsv', 
                        help='Ouput csv file')
    parser.add_argument('-collection', default='Shared,Personal', 
                        help='Collection (default SHARED,PERSONAL)')
...
if __name__ == '__main__':
    ...
    for cole in arg.collection.split(','):
        print( cole )
        url = iceurl+ '/rest/collections/'+cole+'/entries?limit='+str( arg.chunk )
        ...
        while df.shape[0] < total and df.shape[0] < arg.limit:
            print(df.shape[0])
            url = iceurl+ '/rest/collections/'+cole+'/entries?limit='+str( arg.chunk )+'&offset='+str( df.shape[0] )
            response = net_utils.get( url, headers=client._ICEClient__headers )    
